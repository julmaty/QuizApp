...
- Frontend hardcodes backend URL: `http://localhost:8080/api/quizzes` ([frontend/src/App.jsx](frontend/src/App.jsx#L9))
- Backend serves on `:8080` ([backend/main.go](backend/main.go#L11))
- Postgres is the source of truth for quizzes; JSONB column stores serialized `Quiz` structs
- CORS is hardcoded to allow only Vite dev origin: `http://localhost:5173` ([backend/handlers.go](backend/handlers.go#L13))
...
- New HTTP endpoints go in [backend/main.go](backend/main.go#L11) (route registration) + [backend/handlers.go](backend/handlers.go) (implementation)
...
- Ensure POST handlers update both `QuizCreatedAt` **and** generate IDs if missing (see [backend/handlers.go](backend/handlers.go#L60-L68))
